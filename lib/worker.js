...
(function() {
  ...
  module.exports = function(api, validation, couchUtils) {
    var auth, conf, couchClient, emptyResolve, get_gh_role, get_gh_team_id, get_gh_team_ids, get_gh_username, get_or_create_asset, gh, git, handle_add_gh_rsrc_role, handle_add_repo, handle_add_user, handle_create_team, handle_deactivate_user, handle_remove_gh_rsrc_role, handle_remove_repo, handle_remove_user, has_gh_team_membership_through_other_role, teams_api, users;
    ...
    handle_add_user = function(event, team) {
      ...
      user_id = event.user;
      role = event.role;
      return users.getUser(couchClient, user_id, 'promise').then(function(user) {
    ...
    get_gh_team_ids = function(user) {
      return teams_api.getAllTeamRolesForUser(user.name).then(function(team_roles) {
        var gh_team_ids;
